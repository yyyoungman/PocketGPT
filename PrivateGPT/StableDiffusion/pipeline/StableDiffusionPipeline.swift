...
public struct StableDiffusionPipeline: StableDiffusionPipelineProtocol {
    ...
    // unet导出时的batch数，如果是1，则只输入prompt embedding，如果是2，则concat上neg prompt embedding
    var unetBatch = 1
    ...
    public init(
        ...
        self.reduceMemory = true // reduceMemory
    }
...
//        promptEmbedding = try readArrayFromBin(filename: "prompt_embeds.bin", shape: [1, 77, 1024], strides: [78848, 1024, 1])
        
        if unetBatch == 2 {
            // Concatenate the prompt and negative prompt embeddings
            var negativePromptEmbedding = try textEncoder.encode(config.negativePrompt)
        ...
        var hiddenStates = useMultilingualTextEncoder ? promptEmbedding : toHiddenStates(promptEmbedding)
...
//            latents = try [readArrayFromBin(filename: "unet_input_latents.bin", shape: [1, 4, 64, 64], strides: [16384, 4096, 64, 1])]
            // print("cymdebug t: ",t)
            ...
            noise = performGuidance(noise, config.guidanceScale, unetBatch)

            // Have the scheduler compute the previous (t-1) latent
            // sample given the predicted noise and current sample
            for i in 0..<config.imageCount {
//                noise[i] = try readArrayFromBin(filename: "noise_pred.bin", shape: [1, 4, 64, 64], strides: [16384, 4096, 64, 1])
//                latents[i] = try readArrayFromBin(filename: "latents_before_step.bin", shape: [1, 4, 64, 64], strides: [16384, 4096, 64, 1])
//                print("cymdebug t: ",t)
        ...
        // print info of denoisedLatents
        // print("denoisedLatents: \(denoisedLatents)")
        // print("denoisedLatents[0]: \(denoisedLatents[0])")
        // print("denoisedLatents[0].shape: \(denoisedLatents[0].shape)")
        // print("denoisedLatents[0].scalars: \(denoisedLatents[0].scalars)")
        // print("denoisedLatents[0].scalars.count: \(denoisedLatents[0].scalars.count)")
        // print("denoisedLatents[0].scalars[0]: \(denoisedLatents[0].scalars[0])")

        // read denoisedLatents from existing bin file, which was generated by python
//        denoisedLatents = try [readArrayFromBin(filename: "latents_final.bin", shape: [1, 4, 64, 64], strides: [16384, 4096, 64, 1])]

        // Decode the latent samples to images
        return try decodeToImages(denoisedLatents, configuration: config)
    }

    func readArrayFromBin(filename: String, shape: [NSNumber], strides: [NSNumber]) throws -> MLShapedArray<Float32> {
        let modelPath = self.unet.models[0].modelURL
        ...
        return try data.withUnsafeMutableBytes { rawBufferPointer in
            ...
            let mlMultiArray = try MLMultiArray(dataPointer: rawPtr, shape: shape, dataType: .float32, strides: strides, deallocator: nil)
            // convert MLMultiArray to MLShapedArray
            let mlShapedArray = MLShapedArray<Float32>(mlMultiArray)
            return mlShapedArray
        }
...
extension StableDiffusionPipelineProtocol {
    ...
    func performGuidance(_ noise: [MLShapedArray<Float32>], _ guidanceScale: Float, _ unetBatch: Int) -> [MLShapedArray<Float32>] {
        noise.map { performGuidance($0, guidanceScale, unetBatch) }
    }
